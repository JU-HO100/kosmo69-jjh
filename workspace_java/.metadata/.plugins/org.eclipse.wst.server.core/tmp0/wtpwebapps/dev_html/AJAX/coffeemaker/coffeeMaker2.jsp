...
<html>
 <head>
  ...
  <script language="javascript">/* type 과 language의 차이점은? */
   ...
   var request2 = createRequest();
    function createRequest(){
     ...
     function sendRequest(xhrObject, url){//(xhrObject, url) (메소드의 파라미터로 받는다. , url )
     ...
     function serveDrink(){
        ...
        if(request1.readyState == 4){
           if(request1.status == 200){
              //응답 받아오기 ==> 1김유신
              var res = request1.responseText;//1번과 이름을 가져오기때문에 responseText / XML을 받아온다면 responseXML이 된다.
              var machine = res.substring(0,1);//1이순신 일때 0번째에서 1번째 자리까지 자른다. - 1 / 이순신 으로 만든다.
              ...
              if(machine == "1"){
                 //주문한 커피가 나왔으므로 머신 상태를 Idle로 변경함.
                   ...
                   replaceText(coffeemaker1,"Idle");//(el , "Idle")이기 때문에 el의 변수이름 coffeemaker1을 써줬다.
              }
              else{//2번 머신인 경우
           ...
           if(request2.status == 200){
              ...
              var machine = res.substring(0,1);//2
              var name = res.substring(1,res.length);  
              //너 1번 커피 머신이니?
              if(machine == "1"){
                   var coffeemaker1 = document.getElementById("coffmaker1Status");
              ...
              //너 2번 커피 머신이구나
                   ...
                   //Idle ->커피주문이 들어오면 ->~님의 커피를 내리는 중입니다. -> Idle
                   replaceText(coffeemaker2, "Idle");//완료가 됬다면 값을 Idle로 바꿔줘야한다.
              }
              alert(name+", your coffee is ready!");//커피가 나왔다.
