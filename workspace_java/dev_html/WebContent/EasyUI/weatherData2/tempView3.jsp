...
<script>
		...
		$('#sb_temp').searchbox({
		    ...
		    searcher:function(value,ondo){
		       ...
		       var search = $('#sb_temp').searchbox('getValue');
		        alert("  시작연도: "+startYear+"  도착연도: "+endYear+"  입력값: "+value + "  온도조건:  " + ondo + "  구분 값: " + gubun+" 검색 조건: "+ search)
		       		if (startYear>endYear) {
		       			alert('시작연도는 도착연도보다 이른 연도 여야 합니다.');
		       		}
		        	if (endYear!=null && ondo!=null && gubun=='year'){
	  						//------------------------------테이블 값 변경
			        		$('#dg_temp').datagrid({
		        	...
		        	if (startYear!=null && endYear!=null && ondo!=null&& gubun=='years'){
   							//------------------------------테이블 값 변경
			        	if(search=='search'){
			        		$('#dg_temp').datagrid({
        					...
        					//------------------------------테이블 값 변경
// 				        			url:'./getCalTempList.jsp?eYear='+endYear+'&gubun='+ondo+'&search='+search+'&sYear='+startYear
			        	}
					       			alert('확인용'+search);
		        	}//===========================END OF IF
		    }//=================end of searcher
...
<h1> 기온데이터 조회하기</h1>
	<div id=toolbar>
			...
			<div id="mm" style="width:120px">
			    <div id="max" data-options="name:'MAX'">최고기온</div>
			    <div id="min" data-options="name:'MIN'">최저기온</div>
			    <div id="avg" data-options="name:'AVG'">평균기온</div>
			</div>
