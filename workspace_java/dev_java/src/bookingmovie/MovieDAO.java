...
import bookingmovie.Display;
import bookingmovie.MDBConnectionMgr;
...
///////////////////////// 회 원 가 입 insert /////////////////////////
	public void join ( String id, String pw, String memname, String phone ) {
 		...
 		//StringBuilder는 String과 문자열을 더할 때 새로운 객체를 생성하는 것이 아닌
 		//기존의 데이터에 더하는 방식을 사용한다. - 속도가 빠르다.
			...
			while(rs.next()) {
		     ...
		     if(v.size()>0) {
				...
				for(int x=0;x<sVOS.length;x++) {
					...
					oneRow.add(2,sVOS[x].getSEAT_CODE());
