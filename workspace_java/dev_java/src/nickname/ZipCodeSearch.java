...
//         Object obj[]=null;
         	while(rs.next()) {
          	rmap=new HashMap<>();
         		rmap.put("zipcode",rs.getInt("zipCode"));//put
         ...
         //일단 조회 결과가 1건도 없는 경우를 고려한 화면 처리 하기
         Iterator<Map<String, Object>> iter = v.iterator();
		 Object objs[] = null;
         
         if((v==null)||(v.size()<1)) {//null은 항상 조심해야 한다.
//       if((v.size()<1)||(v==null)) {//이렇게 할 경우 앞의 v.size<1의 자리에서 nullpoint 에러를 만난 수도 있다.
        	...
        	return;
        	 ...
        	 while(iter.hasNext()){
        		 ...
        		 objs = date.keySet().toArray();
        		 Vector<Object> oneRow = new Vector<>();
        		 oneRow.add(0, date.get(objs[0]));
        		 oneRow.add(1, date.get(objs[1]));
