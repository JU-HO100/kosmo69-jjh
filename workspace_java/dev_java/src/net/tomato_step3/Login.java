...
//		JButton jbtn_exit   	= new JButton("나가기");
		...
		TomatoMemberShip  tms	= null;
		TomatoDAO		  tdao 	= new TomatoDAO();
		...
		public void actionPerformed(ActionEvent e) {
			...
			String nickName = tdao.procLogin(jtf_id.getText(),jtf_pw.getText());
			...
			if(obj == jbtn_login) {
				if("아이디가 존재하지 않습니다.".equals(nickName)) {
					...
					TomatoDAO bdao = new TomatoDAO();
					bdao.procLogin(mem_id, mem_pw);
					this.dispose();
					tc = new TomatoClient(nickName);
					tc.initDisplay();
					tc.init();
				}
			}
				else if(obj == jbtn_signup) {
					tms = new TomatoMemberShip();
//					tms.initDisplay();
			}
