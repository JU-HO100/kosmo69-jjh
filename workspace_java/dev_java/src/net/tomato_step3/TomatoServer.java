...
public class TomatoServer extends JFrame implements Runnable, ActionListener {
	//선언부
	...
	TomatoServerThread bst 	= null;
	List<TomatoServerThread> globalList = null;/*유저의 정보를 담을 주머니 -globalList-*/
...
//			System.out.println("Server Ready ....\n");
			...
			//43번에서 대기상태에 빠짐으로 run메소드를 먼저 호출할 경우 화면을 볼 기회를 영원히 못 가질수도 있다.	
				...
				bst = new TomatoServerThread(this);//44번이 여길 지나가는 순간 값이 결정된다.
				bst.start(); /*왜 들어가 있을까?*/
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {
		TomatoServer ts = new TomatoServer();//싱글톤
